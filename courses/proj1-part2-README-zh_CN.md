...
关于存储引擎，MySQL version() 支持，，，种存储引擎，默认配置为；PostgreSQL采用[一些引用]；TinySQL 与 TiDB 类似， 也使用 KV 存储引擎作为底层存储。
...
- 从单表操作的角度看，同一张表的数据应当是存放在一起的，这样我们可以避免在处理某一张表的block时，加载其他表上的数据进入内存
